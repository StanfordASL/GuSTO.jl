...
function init_traj_nothing(TOP::TrajectoryOptimizationProblem{PandaBot{T}, PandaKin, E}) where {T,E}
  ...
  X = repeat(0.5(x_init + x_goal),1,N)
  U = zeros(u_dim,N-1)
...
function init_traj_so1(TOP::TrajectoryOptimizationProblem{PandaBot{T}, PandaKin, E}) where {T,E}
  ...
  X = hcat(LinRange(x_init, x_goal, N)...)
...
function ncsbci_EE_goal_constraints(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)
  p_EE_goal = model.p_EE_goal 
  p_EE      = get_EE_position(X[:,N], robot)

  p_EE_goal_max = p_EE_goal[k] + model.p_EE_goal_delta_error
  return p_EE[k] - p_EE_goal_max 
end

function ncsbci_EE_goal_constraints_negative(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)

  p_EE_goal = model.p_EE_goal
  p_EE      = get_EE_position(X[:,N], robot)

  p_EE_goal_min = p_EE_goal[k] - model.p_EE_goal_delta_error
  return p_EE_goal_min - p_EE[k]
end

function ncsi_EE_pointing_constraint(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int, i::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)

  pointing_constraint = get_EE_pointing_constraint(X[:,k],robot) 
  return 1 - model.pointing_tol - pointing_constraint 
end
...
function ncsbci_EE_goal_constraints_convexified(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)
  ...
  p_EE_goal_max = p_EE_goal[k] + model.p_EE_goal_delta_error
  return p_EE[k] + transpose(J_p_EE[k,:]) * (X[:,N]-Xp[:,N]) - p_EE_goal_max 
end

function ncsbci_EE_goal_constraints_negative_convexified(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)
  ...
  p_EE_goal_min = p_EE_goal[k] - model.p_EE_goal_delta_error
  return p_EE_goal_min - (p_EE[k] + transpose(J_p_EE[k,:]) * (X[:,N]-Xp[:,N]))
end

function ncsi_EE_pointing_constraint_convexified(traj, traj_prev::Trajectory, SCPP::SCPProblem{PandaBot{T}, PandaKin, E}, k::Int, i::Int) where {T,E}
  X,U,Tf,Xp,Up,Tfp,dtp,robot,model,WS,x_init,x_goal,x_dim,u_dim,N,dh = @scp_shortcut_PandaKin(traj, traj_prev, SCPP)
  
  pointing_constraint = get_EE_pointing_constraint(Xp[:,k],robot)
  J_pointing_constraint = get_EE_J_pointing_constraint(Xp[:,k],robot)
  return 1 - model.pointing_tol - (pointing_constraint + J_pointing_constraint * (X[:,k]-Xp[:,k]))
end
...
function SCPConstraints(SCPP::SCPProblem{PandaBot{T}, PandaKin, E}) where {T,E}
  ...
  ## Nonconvex state inequality boundary condition constraints
  add_constraint_category!(SCPC.nonconvex_state_bc_ineq, ncsbci_EE_goal_constraints, :scalar, 1:3)
  add_constraint_category!(SCPC.nonconvex_state_bc_ineq, ncsbci_EE_goal_constraints_negative, :scalar, 1:3) 

  ## Nonconvex state inequality boundary condition constraints (convexified)
  add_constraint_category!(SCPC.nonconvex_state_bc_convexified_ineq, ncsbci_EE_goal_constraints_convexified, :scalar, 1:3)
  add_constraint_category!(SCPC.nonconvex_state_bc_convexified_ineq, ncsbci_EE_goal_constraints_negative_convexified, :scalar, 1:3)
  ...
  add_constraint_category!(SCPC.nonconvex_state_ineq, ncsi_obstacle_avoidance_signed_distance, :scalar, 1:N, 1:N_obs, 1:N_rb)
  
  ## Nonconvex state equality constraints (convexified)
  nothing

  ## Nonconvex state inequality constraints (convexified)
  add_constraint_category!(SCPC.nonconvex_state_convexified_ineq, ncsi_obstacle_avoidance_signed_distance_convexified, :scalar, 1:N, 1:N_obs, 1:N_rb)
...
function dynamics_shooting!(Xdot, x, p, u, SP::ShootingProblem{PandaBot{T}, PandaKin, E}) where {T,E}
  robot, model, WS, x_init, x_goal = SP.PD.robot, SP.PD.model, SP.WS, SP.PD.x_init, SP.PD.x_goal
