...
function dynamics_constraints(traj, traj_prev::Trajectory, SCPP::SCPProblem{Astrobee3D{T}, AstrobeeSE3Manifold, E}, k::Int) where {T,E}
  ...
  # Forward Euler rule
  if k == N-1
    return (Xk-Xkn) + dtp.*(fpk + Apk*(Xk-Xpk) + Bpk*(Uk-Upk))
    # Trapezoidal rule
  else
    Ukn = U[:,k+1]
    fpkn, Apkn, Bpkn, Xpkn, Upkn = get_f(k+1, model), get_A(k+1, model), get_B(k+1, model), Xp[:,k+1], Up[:,k+1]
    return (Xk-Xkn) + 1/2*dtp.*(fpk  + Apk *(Xk-Xpk)   + Bpk *(Uk-Upk) +
                                 fpkn + Apkn*(Xkn-Xpkn) + Bpkn*(Ukn-Upkn))
  end
  ...
  #     Xkp, Ukp = X[:,k-1], U[:,k-1]
  #     fpkp, Apkp, Bpkp, Xpkp, Upkp  = get_f(k-1, model), get_A(k-1, model), get_B(k-1, model), Xp[:,k-1], Up[:,k-1]
  #     return (Xkp-Xkn) + 1/6*dtp.*(fpkp + Apkp*(Xkp-Xpkp) + Bpkp*(Ukp-Upkp) +
  #                               4*(fpk  + Apk *(Xk-Xpk)   + Bpk *(Uk-Upk)) +
  #                                  fpkn + Apkn*(Xkn-Xpkn) + Bpkn*(Ukn-Upkn))
  #   end
  # end 
