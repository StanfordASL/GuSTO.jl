...
function add_obstacles4!(env::ISSCorner)
  # ----------
  # RECTANGLES
  ...
  radius = 0.4
  push!(env.obstacle_set, HyperSphere(Point3f0(center), Float32(radius)))
  bottom_left = Vec3f0(+10.00,+5.0,+4.75)
  top_right   = Vec3f0(+10.75,+6.5,+5.75)
  center = ( bottom_left + top_right ) ./ 2.
  radius = 0.375
  push!(env.obstacle_set, HyperSphere(Point3f0(center), Float32(radius)))
  bottom_left = Vec3f0(+10.00,+5.5,+4.75)
...
function add_obstacles5!(env::ISSCorner)
  # ----------
  # RECTANGLES

  bottom_left = Vec3f0(+8.50,-0.00,+4.5)
  top_right   = Vec3f0(+9.25,+0.75,+5.25)
  push!(env.obstacle_set, HyperRectangle(bottom_left, top_right-bottom_left))
  bottom_left = Vec3f0(+11.00,-0.25,+4.75)
  top_right   = Vec3f0(+12.00,+0.75,+5.00)

# Cylinder) = convex_hull_cylinder(SVector{3}(shape.origin), SVector{3}(shape.origin+shape.extremity), shape.r)

  center_down = [+11.35 ; +2.0 ; +4.15 ]
  center_up   = [+11.35 ; +2.0 ; +5.50 ]
  radius = 0.3
  push!(env.obstacle_set, Cylinder(Point3f0(center_down), Point3f0(center_up), Float32(radius)))
